...
if (isset($_POST['booking_code']) && isset($_POST['rejection_reason'])) {
    ...
    // Lấy thông tin đơn đặt sân trước khi cập nhật
    $sql = "
        SELECT
            b.*,
    ...
    if ($result->num_rows === 0) {
        $_SESSION['errors'] = ['Không tìm thấy đơn đặt sân.'];
        header('Location: approve-room.php');
        exit;
    }
    ...
    if ($update_stmt->execute()) {
        ...
        if ($emailSent) {
            ...
            $_SESSION['success'] = 'Đơn đặt sân ' . $booking_code . ' đã bị từ chối! Email thông báo đã được gửi.';
        } else {
            error_log("Failed to send rejection email for booking: " . $booking_code);
            $_SESSION['success'] = 'Đơn đặt sân ' . $booking_code . ' đã bị từ chối! (Email gửi không thành công)';
        }
    } else {
