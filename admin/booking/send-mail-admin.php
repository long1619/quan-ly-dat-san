 ...
 *
 * @param array $bookingData Thông tin đặt sân
...
/**
 * Gửi email phê duyệt đơn đặt sân cho người dùng
 *
 * @param array $bookingData Thông tin đặt sân
 * @return bool True nếu gửi thành công, False nếu thất bại
 */
...
/**
 * Gửi email từ chối đơn đặt sân cho người dùng
 *
 * @param array $bookingData Thông tin đặt sân
...
/**
 * Gửi email hủy đơn đặt sân cho người dùng
 *
 * @param array $bookingData Thông tin đặt sân
...
/**
 * Gửi email xác nhận đặt sân cho người dùng
 *
 * @param array $bookingData Thông tin đặt sân
 * @return void
 */
function sendBookingNotificationToUser($bookingData) {
    ...
    $subject = 'Xác nhận Đặt Sân - ' . $bookingData['booking_code'];
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>Cảm ơn bạn đã Đặt Sân. Dưới đây là thông tin chi tiết về đơn Đặt Sân của bạn:</p>
                    ...
                    <div class='info-row'>
                        ...
                        <span class='value'>" . htmlspecialchars($bookingData['court_code']) . " - " . htmlspecialchars($bookingData['court_name']) . "</span>
                    </div>
...
function sendEmail($recipientEmail, $recipientName, $bookingData, $messageType = 'booking_notification') {
    ...
    try {
        ...
        $mail->Password   = 'hgui vlby xcvv ahhx';
...
function getEmailSubject($messageType, $bookingData) {
    ...
    switch ($messageType) {
        case 'pending':
            return "⏳ Đơn Đặt Sân đang chờ duyệt - {$bookingCode}";
        case 'approval':
            return "✓ Đơn Đặt Sân của bạn đã được phê duyệt - {$bookingCode}";
        case 'rejection':
            return "✕ Đơn Đặt Sân của bạn bị từ chối - {$bookingCode}";
        case 'cancellation':
            return "🚫 Đơn Đặt Sân đã bị hủy - {$bookingCode}";
        case 'booking_notification':
        default:
            return "Thông báo: Đơn Đặt Sân Mới - {$bookingCode}";
    }
}
...
function getEmailTemplate($messageType, $bookingData) {
    ...
    $roomCode = htmlspecialchars($bookingData['court_code']);
    $roomName = htmlspecialchars($bookingData['court_name']);
                        ...
                        <td style="padding: 35px;">
                            ...
                            <div style="background: linear-gradient(135deg, #fff5f7 0%, #fed7e2 100%); border-radius: 12px; padding: 25px; margin-bottom: 25px; border-left: 5px solid #ec4899; box-shadow: 0 4px 12px rgba(236, 72, 153, 0.1);">
                                ...
                                <table style="width: 100%; border-collapse: collapse;">
                                    <tr>
                                        <td style="padding: 14px 0; color: #9f1239; font-size: 15px; width: 40%; font-weight: 600;">mã sân:</td>
                                    ...
                                    <tr style="background-color: rgba(255,255,255,0.4);">
                                        <td style="padding: 14px 0; color: #9f1239; font-size: 15px; font-weight: 600;">tên sân:</td>
                    ...
                    <tr>
                        <td style="background: linear-gradient(135deg, #f7fafc 0%, #edf2f7 100%); padding: 30px; text-align: center; border-top: 2px solid #e2e8f0;">
                            <p style="margin: 0 0 12px 0; color: #4a5568; font-size: 14px; font-weight: 600;">
                                Email này được gửi tự động từ hệ thống quản lý Đặt Sân
                            </p>
...
function getHeaderContent($messageType) {
    $headers = [
        'booking_notification' => [
            ...
            'subtitle' => 'Bạn có một Đơn Đặt Sân mới cần xử lý',
        ...
        'pending' => [
            'title' => 'Đơn Đặt Sân Đang Chờ Duyệt',
            'subtitle' => 'Đơn Đặt Sân của bạn đang trong trạng thái chờ duyệt',
        ...
        'approval' => [
            'title' => 'Đơn Đặt Sân Đã Phê Duyệt',
            'subtitle' => 'Đơn Đặt Sân của bạn đã được chấp nhận',
        ...
        'rejection' => [
            ...
            'subtitle' => 'Đơn Đặt Sân của bạn đã bị từ chối',
        ...
        'cancellation' => [
            ...
            'subtitle' => 'Đơn Đặt Sân của bạn đã bị hủy',
                                ...
                                <div style="display: flex; align-items: start; gap: 12px;">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <p style="margin: 0; color: #78350f; line-height: 1.7; font-size: 15px;">
                                            Đơn Đặt Sân của bạn đã được ghi nhận và đang trong trạng thái chờ duyệt. Quản trị viên sẽ xử lý đơn của bạn sớm nhất có thể.
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div style="display: flex; align-items: start; gap: 12px;">
                                    <div style="flex: 1;">
                                        ...
                                        <p style="margin: 0; color: #065f46; line-height: 1.7; font-size: 15px;">
                                            Sân của bạn đã sẵn sàng sử dụng vào thời gian đã Đặt Sân. Vui lòng kiểm tra lại chi tiết và chuẩn bị sử dụng đúng giờ.
                                        </p>
                                    </div>
                                </div>
                            </div>
        ...
        return <<<HTML
                            ...
                            <div style="background: linear-gradient(135deg, #f3f4f6 0%, #e5e7eb 100%); border-radius: 12px; padding: 25px; margin-bottom: 30px; border-left: 5px solid #6b7280; box-shadow: 0 4px 12px rgba(107, 114, 128, 0.15);">
                                <div style="display: flex; align-items: start; gap: 12px;">
                                    <span style="font-size: 28px;">🚫</span>
                                    <div style="flex: 1;">
                                        <p style="margin: 0 0 8px 0; color: #1f2937; font-weight: 700; font-size: 18px;">Đơn Đặt Sân đã bị hủy</p>
                                        ...
                                        <p style="margin: 0; color: #6b7280; line-height: 1.7; font-size: 14px;">
                                            Nếu bạn cần Đặt Sân mới, vui lòng tạo đơn Đặt Sân khác.
                                        </p>
                                    </div>
                                </div>
                            </div>
