<?php
    ...
    // Kiểm tra quyền đặt sân
    if (!checkPermission($conn, $userRole, 'create_booking')) {
        echo "<script>alert('Bạn không có quyền đặt sân!'); window.location.href='../dashboard/index.php';</script>";
        exit;
    }
    ...
    $selectedRoomId = isset($_GET['court_id']) ? intval($_GET['court_id']) : 0;

    // Kiểm tra nếu court_id từ URL có hợp lệ (active và không bảo trì)
    if ($selectedRoomId > 0) {
        $checkStmt = $conn->prepare("SELECT id, court_code, status, is_active FROM courts WHERE id = ?");
        ...
        if (!$roomCheck || $roomCheck['is_active'] != 1 || $roomCheck['status'] == 'bao_tri') {
            $_SESSION['errors'] = ['Sân không khả dụng để đặt (có thể đã bị vô hiệu hóa hoặc đang bảo trì).'];
            header('Location: ../court/list-court.php');
            exit;
        }
        $checkStmt->close();
    }
    ...
    $sql = "SELECT id, court_code, court_name, capacity FROM courts WHERE is_active = 1 AND status != 'bao_tri' ORDER BY court_code ASC";
...
<body>
                            ...
                            <div class="card-body mx-auto" style="width:70%">
                                ...
                                <form id="bookingForm" method="POST" action="process-booking.php">
                                    ...
                                    <div class="form-section active" id="form-step-1">
                                        <!-- Chọn Phòng -->
                                        <div class="mb-3">
                                            <label class="form-label">Chọn sân <span
                                                    class="text-danger">*</span></label>
                                            <select class="form-select" id="roomSelect" name="court_id">
                                                <option value="">-- Chọn sân --</option>
                                                <?php while ($room = $rooms->fetch_assoc()): ?>
                                                <option value="<?= $room['id'] ?>" data-capacity="<?= $room['capacity'] ?>" <?= ($selectedRoomId == $room['id']) ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($room['court_code']) ?> -
                                                ...
                                                <div class="time-input-group">
                                                    <select class="form-select" id="endTime" name="end_time">
                                                        <?php
                                                            for ($h = 6; $h <= 21; $h++) {
                                                                foreach (['00', '30'] as $m) {
                                                                    if ($h == 6 && $m == '00') continue;
                                                                    $time = sprintf('%02d:%s', $h, $m);
                                                                    $selected = (isset($old['end_time']) && $old['end_time'] == $time) ? 'selected' : '';
                                                                    echo "<option value=\"$time\" $selected>$time</option>";
                                                                }
                                                            }
                                                            $selected2200 = (isset($old['end_time']) && $old['end_time'] == '22:00') ? 'selected' : '';
                                                            echo "<option value=\"22:00\" $selected2200>22:00</option>";
                                                        ?>
                                                    </select>
                                                </div>
                                            </div>
                                    ...
                                    <div class="form-section" id="form-step-3">
                                        <!-- Info Box -->
                                        <div class="info-box">
                                            <div class="info-box-title">Thông tin đặt sân</div>
                                            <div class="info-row">
    ...
    function updateStepUI() {
        ...
        if (currentStep === totalSteps) {
            btnNext.innerHTML = 'Xác nhận đặt sân';
        } else {
        ...
        if (step === 1) {
            ...
            if (startTime < "06:00" || endTime > "22:00") {
                Swal.fire({
                    icon: 'warning',
                    title: 'Thời gian không hợp lệ',
                    text: 'Chỉ cho phép đặt sân từ 06:00 sáng đến 22:00 tối'
                });
                return false;
            }
    ...
    async function handleNext() {
        ...
        if (currentStep === 1) {
            ...
            try {
                const response = await fetch(`check-room-availability.php?court_id=${room}&date=${date}&start_time=${startTime}&end_time=${endTime}`);
