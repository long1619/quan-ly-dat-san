<?php
    ...
    $rooms_sql = "SELECT id, court_code, court_name, area, surface_type, capacity, status, facilities FROM courts WHERE is_active = 1 ORDER BY court_code ASC";
...
<body>
                                ...
                                <div class="room-selector">
                                    <select id="roomSelect" onchange="updateSchedule()">
                                        ...
                                        <option value="<?= $room['id'] ?>" <?= $room['id'] == $selectedRoomId ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($room['court_code']) ?> - <?= htmlspecialchars($room['court_name']) ?>
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="room-info-card">
                                    ...
                                    <div class="room-info-item">
                                        <span class="room-info-label">Sân:</span>
                                    ...
                                    <button class="btn-book-time" onclick="openBookingModal()">đặt sân</button>
                                </div>
                            </div>
        ...
        times.forEach(time => {
            ...
            for (let i = 0; i < numCols; i++) {
                ...
                if (booking) {
                    ...
                    // Chỉ vẽ booking nếu đây là giờ bắt đầu (hoặc giờ đầu tiên của lịch hiển thị là 6:00)
                    if (currentHour === bookingStartHour || (currentHour === 6 && bookingStartHour < 6)) {
                        let displayStartHour = Math.max(6, bookingStartHour);
    ...
    function renderBookingSlot(booking, date, time) {
        if (booking) {
            ...
            const title = booking.purpose || 'đặt sân';
            ...
            return `<div class="booking-slot available"
                        ...
                        title="Sân trống - Click để đặt"></div>`;
        }
    }
    ...
    function updateRoomInfo() {
        ...
        if (info) {
            document.getElementById('roomName').textContent = info.court_code + ' - ' + info.court_name;
            const location = info.area + (info.surface_type ? ' (' + info.surface_type + ')' : '');
    ...
    function bookTimeSlot(dateStr, time) {
        const roomId = document.getElementById('roomSelect').value;
        window.location.href = `booking-room.php?court_id=${roomId}&date=${dateStr}&time=${time}`;
    }
    ...
    function openBookingModal() {
        ...
        const roomId = document.getElementById('roomSelect').value;
        window.location.href = `booking-room.php?court_id=${roomId}&date=${selectedDate.toISOString().split('T')[0]}`;
    }
