...
$booking_code = 'BK' . date('YmdHis') . rand(100, 999); // Tạo mã đặt sân tự động
...
if ($start_time < '06:00' || $end_time > '22:00') {
    $errors[] = 'Chỉ cho phép đặt sân từ 06:00 sáng đến 22:00 tối.';
}
...
if (empty($errors)) {
    $stmt = $conn->prepare("SELECT court_name, capacity FROM courts WHERE id = ? AND is_active = 1");
    $stmt->bind_param("i", $court_id);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 0) {
        $errors[] = 'Sân không tồn tại hoặc không hoạt động.';
    } else {
        $room = $result->fetch_assoc();

        // Kiểm tra số người tham gia có vượt quá sức chứa không
        if ($participants > $room['capacity']) {
            $errors[] = 'Số người tham gia (' . $participants . ') vượt quá sức chứa của sân (' . $room['capacity'] . ').';
        }
    }
    $stmt->close();
}
...
if (empty($errors)) {
    $stmt = $conn->prepare("
        ...
        AND end_time > ?
    ");
    $stmt->bind_param("isss", $court_id, $booking_date, $end_time, $start_time);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $errors[] = 'Sân đã được đặt trong khoảng thời gian này. Vui lòng chọn thời gian khác.';
    }
    $stmt->close();
}

// Nếu có lỗi, quay lại trang đặt sân
if (!empty($errors)) {
...
try {
    ...
    $stmt = $conn->prepare("
        ...
        (booking_code, court_id, user_id, booking_date, start_time, end_time, purpose, participants, contact_phone, notes, status, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    ...
    if ($booking_inserted) {
        ...
        */
        ...
        $sql_booking_info = "
            ...
            FROM bookings b
            INNER JOIN courts r ON b.court_id = r.id
        ...
        $_SESSION['success'] = 'Đặt sân thành công! Mã đặt sân: ' . $booking_code . '. Vui lòng chờ phê duyệt.';
    ...
    $_SESSION['errors'] = ['Lỗi khi đặt sân: ' . $e->getMessage()];
    $_SESSION['old'] = $_POST;
    header('Location: booking-room.php');
    exit;
}
